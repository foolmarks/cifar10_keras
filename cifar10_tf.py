...
def cnn(x):
  ...
  logits = tf.layers.dense(inputs=fc1, units=10, kernel_initializer=tf.glorot_uniform_initializer(), activation=None, name='logits')
  return logits
...
with tf.Session() as sess:
    ...
    # save checkpoint & graph file as binary protobuf
    save_path = saver.save(sess, os.path.join(CHKPT_DIR, CHKPT_FILE))
    tf.train.write_graph(sess.graph_def, CHKPT_DIR, GRAPH_FILE, as_text=False)

#  Session ended


# freeze the saved graph - converts variables to constants & removes training nodes
freeze_graph.freeze_graph(input_graph=os.path.join(CHKPT_DIR, GRAPH_FILE),
                          input_saver='',
                          input_binary = True,
